...
export default function StudentTaskEvaluationPage() {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      try {
        ...
        const res = await apiRequest<{ data?: any[] }>(`/video/student/${studentId}/history`);
        if (isMounted && res?.data && Array.isArray(res.data) && res.data.length > 0) {
          ...
          setData({
            ...
            videoTitle: matched.videoTitle || "Practice Submission",
  ...
  const videoSourceUrl = getFullVideoUrl(data?.fileUrl);
  ...
  return (
    <div className="max-w-[1240px] mx-auto space-y-8 font-sans pb-20 text-stone-800 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-xs font-extrabold text-stone-400 uppercase tracking-wider">
            <Link href="/student/video-submission" className="hover:text-[#8B1D2C] transition-colors">
              ASSIGNMENTS
            </Link>
            <span>›</span>
          ...
          <h1 className="text-2xl sm:text-4xl font-extrabold text-stone-900 tracking-tight">
            {formatDisplayTitle(data?.videoTitle)}
          </h1>
        ...
        <div className="shrink-0">
          <Link
            href="/student/video-submission/upload"
      ...
      {/* TOP GRID: Real Video Player Left | Overall Performance Card Right */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
        
        {/* REAL VIDEO PLAYER CONTAINER (8 cols) */}
          ...
          {videoSourceUrl ? (
            <div className="w-full h-full flex flex-col justify-between min-h-[360px] relative">
              ...
              <div className="p-3 bg-stone-900/90 rounded-xl mt-2 flex items-center justify-between text-xs text-stone-300 font-mono">
                <span>TASK: {formatDisplayTitle(data?.videoTitle)}</span>
                <span className="text-emerald-400 font-bold">SUBMITTED VIDEO</span>
              </div>
            </div>
          ) : (
            <div className="w-full h-full py-20 flex flex-col items-center justify-center space-y-2 text-stone-400">
              <AlertCircle className="w-8 h-8 text-amber-400" />
              <span className="text-xs font-semibold">No video file available for playback.</span>
            </div>
          )}
        </div>
          ...
          {data?.feedbackNotes ? (
            ...
            <div className="bg-stone-50 rounded-2xl p-6 border border-stone-200/80 text-center space-y-2">
              ...
              <p className="text-xs text-stone-500 font-medium max-w-md mx-auto">
                Your practice video "{formatDisplayTitle(data?.videoTitle)}" has been received successfully and is currently in queue for Guru review.
              </p>
            </div>
          )}
