...
// Viewer History Interface added
...
interface RecordedClassDetail {
  ...
  viewHistory?: ViewHistory[]; // Analytics data array
}
...
export default function AdminRecordedClassDetailPage() {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      ...
      try {
        ...
        if (res.data?.classes) {
          const found = res.data.classes.find((c) => c.id === classId);
          if (found) setClassData(found);
        } else if (res.data?.recordedClass) {
          setClassData(res.data.recordedClass);
        }
      } catch (err) {
      ...
      <div className="flex items-center justify-center min-h-[70vh]">
        <div className="flex flex-col items-center gap-4 bg-white/50 p-8 rounded-3xl border border-stone-100 shadow-sm backdrop-blur-sm">
          <div className="relative">
            ...
            <Loader2 className="w-12 h-12 animate-spin text-[#9E0C25] relative z-10" />
          </div>
          <span className="text-sm font-bold text-stone-500 tracking-wide uppercase">Loading Details...</span>
        </div>
      </div>
    );
  }
  ...
  if (!classData) {
    return (
      <div className="max-w-lg mx-auto py-20 text-center space-y-5 bg-white rounded-[2rem] border border-stone-200/60 shadow-xl p-10 mt-12">
        <div className="w-20 h-20 rounded-full bg-rose-50 text-[#9E0C25] flex items-center justify-center mx-auto shadow-inner">
          <Film className="w-10 h-10" />
        </div>
        <div>
          <h2 className="text-2xl font-black text-stone-900 mb-2">Session Not Found</h2>
          <p className="text-sm text-stone-500 leading-relaxed">
            The recorded class you are looking for might have been removed or the URL is incorrect.
          </p>
        </div>
        <button
          onClick={() => router.push("/admin/recorded-class")}
          className="w-full py-3.5 rounded-2xl bg-[#9E0C25] hover:bg-[#800A1E] text-white text-sm font-bold transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 cursor-pointer"
        >
  ...
  const analyticsData = classData.viewHistory || [];
  ...
  return (
    <div className="space-y-8 max-w-[1100px] mx-auto pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="sticky top-0 z-10 py-4 bg-stone-50/80 backdrop-blur-md border-b border-stone-200/50 -mx-4 px-4 sm:mx-0 sm:px-0 sm:bg-transparent sm:backdrop-blur-none sm:border-none sm:py-0 sm:static">
        <button
          onClick={() => router.push("/admin/recorded-class")}
          className="group inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full bg-white border border-stone-200/80 shadow-sm text-xs font-bold text-stone-600 hover:text-[#9E0C25] hover:border-rose-200 transition-all cursor-pointer"
        >
      ...
      <div className="bg-black rounded-[0.5rem] overflow-hidden shadow-2xl shadow-[#9E0C25]/5 border border-stone-200/50 aspect-video w-full relative group">
      ...
      <div className="bg-white rounded-[1rem] p-6 sm:p-10 border border-stone-200/60 shadow-lg shadow-stone-200/40 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 pb-8 border-b border-stone-100">
          <div className="space-y-4 flex-1">
            <div className="flex flex-wrap items-center gap-2">
              <span className="px-3.5 py-1.5 rounded-xl bg-rose-50 text-[#9E0C25] text-[10px] font-extrabold uppercase tracking-wider border border-rose-100 flex items-center gap-1.5 shadow-xs">
                <Sparkles className="w-3.5 h-3.5" />
                {classData.course?.title || "Kathak Course"}
              </span>
          ...
          <div className="flex flex-wrap md:flex-col items-center md:items-end gap-3 shrink-0">
            <div className="flex items-center gap-2 text-xs font-bold text-blue-700 bg-blue-50 border border-blue-200/80 px-4 py-2.5 rounded-2xl shadow-xs w-full justify-center md:justify-start">
              <Eye className="w-4 h-4 text-blue-600" />
              {classData.viewsCount || 0} Total Views
            </div>
        ...
        <div className="flex items-center bg-stone-100/80 p-1.5 rounded-lg w-fit border border-stone-200/50 overflow-x-auto">
          <button
            onClick={() => setActiveTab("overview")}
            className={`px-6 py-2.5 rounded-xl text-xs font-extrabold transition-all duration-300 cursor-pointer whitespace-nowrap ${
              activeTab === "overview"
                ? "bg-white text-[#9E0C25] shadow-sm ring-1 ring-stone-200/50"
          ...
          <button
            ...
            className={`px-6 py-2.5 rounded-xl text-xs font-extrabold transition-all duration-300 cursor-pointer flex items-center gap-2 whitespace-nowrap ${
              activeTab === "resources"
                ? "bg-white text-[#9E0C25] shadow-sm ring-1 ring-stone-200/50"
                : "text-stone-500 hover:text-stone-700 hover:bg-stone-200/50"
            }`}
          >
            ...
            {resources.length > 0 && (
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-black transition-colors ${
                activeTab === "resources" ? "bg-rose-100 text-[#9E0C25]" : "bg-stone-200 text-stone-500"
              }`}>
                {resources.length}
              </span>
            )}
          ...
          >
            ...
            {analyticsData.length > 0 && (
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-black transition-colors ${
                activeTab === "analytics" ? "bg-rose-100 text-[#9E0C25]" : "bg-stone-200 text-stone-500"
              }`}>
            ...
            <div className="space-y-8 animate-in slide-in-from-left-4 fade-in duration-300">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-3">
                  ...
                  <div className="text-sm font-medium text-stone-700 leading-relaxed whitespace-pre-line bg-stone-50/50 p-6 sm:p-8 rounded-3xl border border-stone-200/60 shadow-inner shadow-stone-100/50 h-full">
                    {classData.description || (
                      <span className="italic text-stone-400">No detailed description provided.</span>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-stone-50/80 p-6 rounded-3xl border border-stone-200/60 flex flex-col gap-4 h-full">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`text-xs font-bold px-3 py-1.5 rounded-lg border shadow-sm flex items-center gap-1.5 ${
                        ...
                        {classData.isPublic ? <Globe className="w-3.5 h-3.5" /> : <Lock className="w-3.5 h-3.5" />}
                        {classData.isPublic ? "Public" : "Private"}
                      </span>
                    </div>

                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-stone-500">Downloads</span>
                      <span className={`text-xs font-bold px-3 py-1.5 rounded-lg border shadow-sm flex items-center gap-1.5 ${
                        classData.isDownloadable 
                          ? "bg-blue-50 text-blue-700 border-blue-200" 
                          : "bg-rose-50 text-rose-700 border-rose-200"
                      }`}>
                        {classData.isDownloadable ? <DownloadCloud className="w-3.5 h-3.5" /> : <X className="w-3.5 h-3.5" />}
                        {classData.isDownloadable ? "Allowed" : "Restricted"}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3 pt-2">
                  ...
                  <div className="flex flex-wrap items-center gap-2.5">
                    {classData.tags.map((tag, idx) => (
                      <span
                        key={idx}
                        className="px-4 py-2 rounded-xl bg-white border border-stone-200 shadow-sm hover:border-[#9E0C25]/30 hover:shadow-md hover:-translate-y-0.5 text-stone-700 text-xs font-bold transition-all flex items-center gap-2 cursor-default"
                      >
                        <div className="w-1.5 h-1.5 rounded-full bg-[#9E0C25]" />
                        {tag}
                      </span>
                    ))}
                  </div>
            ...
            <div className="space-y-4 animate-in slide-in-from-right-4 fade-in duration-300">
              {resources.length === 0 ? (
                <div className="py-16 text-center space-y-4 bg-stone-50/50 rounded-3xl border border-stone-200/60 border-dashed">
                  <div className="w-16 h-16 bg-stone-100 rounded-full flex items-center justify-center mx-auto shadow-inner">
                    <BookOpen className="w-8 h-8 text-stone-300" />
                  </div>
                  <div>
                    <h4 className="font-bold text-stone-800">No resources available</h4>
                    <p className="text-xs text-stone-500 mt-1 max-w-sm mx-auto">
                      There are currently no notes, PDFs, or audio tracks attached to this masterclass.
                    </p>
                  </div>
                </div>
              ) : (
                  ...
                  {resources.map((res, idx) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4 relative z-10">
                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 shadow-inner transition-transform duration-300 group-hover:scale-105 ${res.type === "audio" ? "bg-gradient-to-br from-purple-100 to-purple-50 text-purple-600 border border-purple-100" : "bg-gradient-to-br from-rose-100 to-rose-50 text-[#9E0C25] border border-rose-100"}`}>
                          {res.type === "audio" ? <Music className="w-6 h-6" /> : <FileText className="w-6 h-6" />}
                        </div>
                        <div className="pr-4">
                          <h5 className="font-bold text-sm text-stone-900 group-hover:text-[#9E0C25] transition-colors truncate max-w-[200px] sm:max-w-[280px]">{res.name}</h5>
                          <p className="text-xs text-stone-400 font-medium mt-1 flex items-center gap-1.5">
                            {res.url ? <><span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />Click to view</> : "Preview unavailable"}
                          </p>
                        </div>
                      </div>
                      
                      {res.url && (
                        <div className="w-10 h-10 rounded-full bg-stone-50 border border-stone-200 flex items-center justify-center group-hover:bg-[#9E0C25] group-hover:text-white group-hover:border-[#9E0C25] group-hover:rotate-12 transition-all duration-300 shrink-0 relative z-10 shadow-sm">
  ...
  <div className="space-y-6 animate-in slide-in-from-right-4 fade-in duration-300">
    ...
    <div className="flex items-center justify-between">
       ...
       <span className="text-xs font-bold bg-stone-100 text-stone-600 px-3 py-1.5 rounded-lg border border-stone-200">
          Total Unique Viewers: {analyticsData.length}
       </span>
    </div>

    {analyticsData.length === 0 ? (
      <div className="py-16 text-center space-y-4 bg-stone-50/50 rounded-2xl border border-stone-200/60 border-dashed">
        <div className="w-16 h-16 bg-stone-100 rounded-full flex items-center justify-center mx-auto shadow-inner">
          <Eye className="w-8 h-8 text-stone-300" />
        </div>
        <div>
          <h4 className="font-bold text-stone-800">No views recorded yet</h4>
          <p className="text-xs text-stone-500 mt-1 max-w-sm mx-auto">
            Analytics will appear here once students start watching this class.
          </p>
        </div>
      </div>
    ) : (
      <div className="bg-white rounded-xl border border-stone-200 overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-stone-50 text-stone-500 font-extrabold text-[10.5px] uppercase tracking-wider border-b border-stone-200">
              <tr>
                <th className="px-5 py-3.5">Student</th>
                <th className="px-5 py-3.5">Device</th>
                <th className="px-5 py-3.5">Location</th>
                <th className="px-5 py-3.5">Last Viewed</th>
              </tr>
            </thead>
                ...
                <tr key={record.id} className="hover:bg-stone-50/60 transition-colors">
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-full bg-rose-50 text-[#9E0C25] flex items-center justify-center shrink-0">
                        <UserCircle className="w-4 h-4" />
                      </div>
                      <div className="min-w-0">
                        <p className="font-bold text-stone-900 text-xs truncate">{record.user.name || record.user.email}</p>
                        {record.user.name && (
                          <p className="text-[10.5px] text-stone-400 truncate">{record.user.email}</p>
                        )}
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-3.5">
                    <span className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-md bg-stone-100 text-stone-600 font-semibold">
                      <MonitorSmartphone className="w-3.5 h-3.5 text-stone-400" />
                  ...
                  <td className="px-5 py-3.5 text-xs font-semibold text-stone-500">
            ...
            <div className="flex items-center justify-between px-8 py-5 border-b border-stone-100 shrink-0 bg-white relative z-10">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-inner ${
                  viewerResource.type === 'audio' ? 'bg-purple-50 text-purple-600' : 'bg-rose-50 text-[#9E0C25]'
                }`}>
              ...
              <button
                onClick={() => setViewerResource(null)}
                className="w-10 h-10 rounded-full bg-stone-100 hover:bg-rose-100 hover:text-[#9E0C25] text-stone-500 flex items-center justify-center transition-all cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
