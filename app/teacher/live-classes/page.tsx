...
export default function TeacherLiveClassesPage() {
  ...
  const fetchClasses = useCallback(async () => {
    try {
      const res = await apiRequest<{ status: string; data: { classes: LiveClass[] } }>(ENDPOINTS.LIVE_CLASS_TEACHER);
  ...
  // ✅ Class abhi join karne layak hai ya nahi — status + actual scheduledEnd time dono check karta hai
    ...
    return (
      <div className="min-h-screen bg-white p-6 flex justify-center items-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-gray-500 font-medium text-sm">Loading Classes...</p>
        </div>
      </div>
    );
  }
  ...
  // Hero Card Class (Agar koi live hai toh wo dikhega, warna sabse pehli upcoming class)
  const heroClass = liveClasses.length > 0 ? liveClasses[0] : upcomingClasses[0];

  // Helper for Date formatting (e.g., "JUL", "20")
  ...
  const setClassStatus = async (id: string, status: "LIVE" | "COMPLETED", joinAfter = false) => {
    setActingId(id);
    try {
      await apiRequest(`${ENDPOINTS.LIVE_CLASS_TEACHER}/${id}/status`, {
        method: "PATCH",
        body: JSON.stringify({ status }),
      });
      setClasses((prev) => prev.map((c) => (c.id === id ? { ...c, status } : c)));
      if (joinAfter) router.push(`/teacher/live-classes/room/${id}`);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-white p-6 lg:p-8 font-sans flex justify-center">
      <div className="w-full max-w-[1200px] mx-auto">
        ...
        {/* 2. Top Stats Row */}
        <div className="w-full border-b border-stone-200 pb-8 mb-8 flex flex-wrap items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-full bg-rose-50 text-[#9B3434] flex items-center justify-center border border-rose-100">
              <CheckCircle2 className="w-5 h-5" />
            </div>
            <div className="flex flex-col">
              <span className="text-[#464555] text-[14px] font-medium leading-[20px]">Completed<br />Classes</span>
              <span className="text-[#0B1C30] text-[24px] font-bold leading-[31.2px]">{completedClasses.length}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              ...
              <span className="flex items-baseline gap-2">
                <span className="text-[#0B1C30] text-[24px] font-bold leading-[31.2px]">{upcomingClasses.length}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-[24px]">
          ...
          <div className="lg:col-span-8 flex flex-col gap-8">
            ...
            {heroClass ? (
              <div className="relative w-full rounded-[24px] overflow-hidden bg-stone-900 shadow-[0_8px_30px_rgba(0,0,0,0.12)]">
                ...
                <div className="relative z-10 p-8 flex flex-col justify-end h-full min-h-[340px]">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                    ...
                    {heroClass.status === "LIVE" ? (
                      <div className="flex items-center gap-3">
                        <Link href={`/teacher/live-classes/room/${heroClass.id}`} className="bg-white hover:bg-stone-100 text-[#9B3434] px-6 py-3.5 rounded-xl font-bold text-[14px] transition-colors shadow-lg flex items-center justify-center gap-2">
              ...
              <div className="w-full bg-stone-50 rounded-[24px] border-2 border-dashed border-stone-200 min-h-[340px] flex flex-col items-center justify-center text-stone-400">
            ...
            <div>
              ...
              <div className="space-y-4">
                {completedClasses.length > 0 ? completedClasses
                  ...
                  .map((session) => (
                    <div key={session.id} className="bg-stone-50 border border-stone-100 rounded-[20px] p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="flex items-center gap-4">
                        <div className="w-[60px] h-[60px] rounded-[16px] bg-white border border-stone-200 flex flex-col items-center justify-center text-stone-500 shrink-0">
                          <span className="text-[11px] font-bold uppercase">{formatMonth(session.scheduledStart)}</span>
                          <span className="text-[20px] font-bold leading-none mt-0.5">{formatDay(session.scheduledStart)}</span>
                        </div>
                        <div>
          ...
          <div className="lg:col-span-4 flex flex-col gap-8">
            ...
            <div className="bg-gradient-to-br from-[#0284C7] to-[#0369A1] rounded-[24px] p-6 text-white shadow-lg shadow-sky-900/20 relative overflow-hidden">
              <h4 className="text-[15px] font-bold mb-4 flex items-center gap-2" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                ...
                &quot;Start the class when you are ready. Students can join only after you go live.&quot;
              </p>
            </div>
